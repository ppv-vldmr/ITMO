...
public class Solve {
    ...
    public static class Bias extends Node {
        ...
        public Bias(TensorSize tensorSize, MyReader myReader) {
            bias = Vector.generate(tensorSize.depth, 0);
            for (int i = 0; i < tensorSize.depth; i++) {
                bias.apply(i, second, (double) myReader.nextInt());
            }
            inputSize = tensorSize.copy();
            outputSize = tensorSize.copy();
    ...
    public abstract static class Cnv extends Node {
        ...
        public Cnv(int cnt, int n, int m, int p, int s, TensorSize inputSize, MyReader myReader) {
                ...
                for (Matrix matrix : kernel.in) {
                    for (Vector vector : matrix.in) {
                        for (int i = 0; i < vector.in.size(); i++) {
                            vector.apply(i, second, (double) myReader.nextInt());
                        }
                    }
                }
            }
    ...
    public static class Cnve extends Cnv {
        ...
        public Cnve(int cnt, int n, int m, int p, int s, TensorSize inputSize, MyReader myReader) {
            super(cnt, n, m, p, s, inputSize, myReader);
        }
    ...
    public static class Cnvc extends Cnv {
        ...
        public Cnvc(int cnt, int n, int m, int p, int s, TensorSize inputSize, MyReader myReader) {
            super(cnt, n, m, p, s, inputSize, myReader);
        }
    ...
    public static class Cnvm extends Cnv {
        ...
        public Cnvm(int cnt, int n, int m, int p, int s, TensorSize inputSize, MyReader myReader) {
            super(cnt, n, m, p, s, inputSize, myReader);
        }
    ...
    public static class Network {
        ...
        public Network(TensorSize size, MyReader myReader) {
            this.size = size;
        ...
        public void addBias() {
            nodes.add(new Bias(getSize(), myReader));
        }
        ...
        public void addCnv(int cnt, int n, int m, int p, int s, CnvType type) {
            switch (type) {
                case E:
                    nodes.add(new Cnve(cnt, n, m, p, s, getSize(), myReader));
                    break;
                case M:
                    nodes.add(new Cnvm(cnt, n, m, p, s, getSize(), myReader));
                    break;
                case C:
                    nodes.add(new Cnvc(cnt, n, m, p, s, getSize(), myReader));
                    break;
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        Tensor t = Tensor.generate(d, n, n, 0);
        foo();
        for (int i = 0; i < d; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < n; k++) {
                    t.get(i).get(j).apply(k, second, (double) myReader.nextInt());
                }
            }
        }
        Network network = new Network(new TensorSize(d, n, n), myReader);
        ...
        t.print(printWriter);
        int a1 = t.in.size();
        int a2 = t.get(0).in.size();
        int a3 = t.get(0).get(0).in.size();
