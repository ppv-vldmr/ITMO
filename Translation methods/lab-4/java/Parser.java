...
public class Parser {
    ...
    if (token.type() != LPAR)
        throw new RuntimeException("Expected LPAR, but found " + token.type().name());
                                    addChild(token);
                                    nextToken();
                                    var child_0_2 = new Tree_init_ZERO_OR_ONE112513540(
                                    );
                                    child_0_2.parse();
                                    addChild(child_0_2);

                                    var init_ZERO_OR_ONE112513540 = child_0_2;
                                        var init = init_ZERO_OR_ONE112513540;
    if (token.type() != SEMICOLON)
        throw new RuntimeException("Expected SEMICOLON, but found " + token.type().name());
                                    addChild(token);
                                    nextToken();
                                    var child_0_4 = new Tree_cond_ZERO_OR_ONE1701244521(
                                    );
                                    child_0_4.parse();
                                    addChild(child_0_4);

                                    var cond_ZERO_OR_ONE1701244521 = child_0_4;
                                        var cond = cond_ZERO_OR_ONE1701244521;
    if (token.type() != SEMICOLON)
        throw new RuntimeException("Expected SEMICOLON, but found " + token.type().name());
                                    addChild(token);
                                    nextToken();
                                    var child_0_6 = new Tree_inc_ZERO_OR_ONE1813817053(
                                    );
                                    child_0_6.parse();
                                    addChild(child_0_6);

                                    var inc_ZERO_OR_ONE1813817053 = child_0_6;
                                        var inc = inc_ZERO_OR_ONE1813817053;
    if (token.type() != RPAR)
    ...
    if (token.type() != VAR)
        ...
        public class Tree_init_ZERO_OR_ONE112513540 implements Tree {
        ...
        public class Tree_cond_ZERO_OR_ONE1701244521 implements Tree {
        ...
        public class Tree_inc_ZERO_OR_ONE1813817053 implements Tree {
